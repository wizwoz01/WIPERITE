...
FILES_${PN} = "/usr/bin/DOOM.WAD \
	          "
S = "${WORKDIR}"

do_compile() {
    oe_runmake CC="${CC}" LD="${LD}" CFLAGS="${CFLAGS}" LDFLAGS="${LDFLAGS}"
}

do_install() {
	     install -d ${D}${bindir}
	     install -m 0755 fbdoom ${D}${bindir}
	     install -D -m 0644 ${WORKDIR}/DOOM.WAD ${D}${bindir}/DOOM.WAD 
}
