...
static void show_menu(void){
	...
	UART0_OutString((uint8_t *)"M/m : Show this menu\r\n");
	UART0_OutString((uint8_t *)"T/t : PCA9685 register test (prints MODE1/PRESCALE, toggles CH8)\r\n");
	UART0_OutString((uint8_t *)"P/p : Print current BASE/ARM/PEN angles\r\n");
	UART0_OutString((uint8_t *)"V/v : Dump mapping samples (corners + center)\r\n");
	UART0_OutString((uint8_t *)"\r\nAwaiting command> ");
}
...
int main(void){
	...
	for(;;){
		...
		if((UART0_FR_R & UART_FR_RXFE) == 0){
			...
			case 'P': case 'p':{
				UART0_OutString((uint8_t *)"Angles: BASE=");
				UART0_OutUDec((uint32_t)baseAngle);
				UART0_OutString((uint8_t *)" ARM=");
				UART0_OutUDec((uint32_t)armAngle);
				UART0_OutString((uint8_t *)" PEN=");
				UART0_OutUDec((uint32_t)penAngle);
				UART0_OutString((uint8_t *)"\r\n");
			} break;

			case 'V': case 'v':{
				UART0_OutString((uint8_t *)"Dumping mapping samples...\r\n");
				WritingArm_DumpMappingSamples();
			} break;
